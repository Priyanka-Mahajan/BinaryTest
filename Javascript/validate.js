...
function validateInputField(input) {
    ...
    if(input.value == "" || input.value == null){
        ...
        if(sum > 300){
            $(input).tooltip({ 'title': 'Sum of all the pipe angles should be less than ' + 300 + ' degrees.'}).tooltip('show');
            errorFlag = true;
        }
    }
    ...
    if(!errorFlag){
        ...
        setTimeout(function(){ 
            ...
            unityInstance.SendMessage('ControlMaster', 'SendParameters', JSON.stringify(param)); 

            //disable update spinner
