...
function addPipeDiaField(pipeCount) {
    ...
    for(let i = 1; i <= pipeCount; i++)
    {
        ...
        inputGrp.innerHTML = '<label>Pipe ' + i + ' (mm)</label><div class="inputDiv"><input id="Diameter' + i + '" name="DIAMETERS|DIA' + i + '" autocomplete="off" value="' + val + '" min="200" max="900" step="0.01" data-toggle="tooltip" data-placement="auto" onblur="InputFocusLost(this)" onchange="validateInputField(this)" onkeydown="validateKeyStroke(this)" required></div>';
...
function addPipeAngleField(pipeCount) {
    ...
    for(let i = 1; i < pipeCount; i++)
    {
        ...
        inputGrp.innerHTML = '<label>Between Pipe ' + i + '-' + (i + 1) + ' (deg)</label><div class="inputDiv"><input id="Angle' + i + '" name="ANGLES|ANGLE' + i + '" autocomplete="off" value="' + val + '" min=' + min + ' max=' + max + ' step="0.01" data-toggle="tooltip" data-placement="auto" onblur="InputFocusLost(this)" onchange="validateInputField(this)" onkeydown="validateKeyStroke(this)" required></div>';
